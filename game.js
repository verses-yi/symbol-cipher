// Symbol Cipher - Game with 2,998 Real Words
...
function handleSkipOrNext() {
  if (gameState.solved) nextPuzzle();
  else skipPuzzle();
}
...
function setMode(newMode) {
  ...
  if (saved?.text) {
    ...
    renderPuzzle();
    updateAlphabet();
    updateStatus(saved.solved ? 'Solved!' : 'Tap a symbol, then a letter');
    updateSkipButton();
  } else {
    newPuzzle();
  }
...
function updateSkipButton() {
  ...
  if (btn) btn.textContent = gameState.solved ? 'Next ➔' : '⏭ Skip';
}
...
function selectSymbol(letter) {
  ...
  document.querySelectorAll('.symbol').forEach(el => {
    ...
    const expectedSymbol = gameState.symbolMap[letter];
    if (el.textContent === expectedSymbol && !el.classList.contains('revealed')) {
...
function updateAlphabet() {
  document.querySelectorAll('.letter').forEach(btn => {
    ...
    btn.classList.toggle('disabled', Object.values(gameState.userMappings).includes(letter));
  });
}
